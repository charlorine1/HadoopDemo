...
import com.usst.etl.util.LoggerUtil;


/**
   *       从hdfs 读取数据，通过map来清洗，然后导入到hbase
 * 
 * */
public class AnalyserLogDataMapper extends Mapper<LongWritable, Text, LongWritable, Put> {
	...
	protected void map(LongWritable key, Text value, Context context)
		...
		try {
			...
			case EVENT:
				// 处理数据
				this.handleData(clientInfo, event, context,key);
				break;
			default:
				this.filterRecords++;
	...
	private void handleData(Map<String, String> clientInfo, EventEnum event,
			Context context, LongWritable key) throws IOException, InterruptedException {
