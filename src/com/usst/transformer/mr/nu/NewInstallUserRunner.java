...
import com.usst.commom.EventLogConstants.EventEnum;
...
//    	conf.set("yarn.resourcemanager.hostname", "node3");
	...
	/**
	 * 创建过滤器： 过滤获取要分析的当天hbase数据 过滤获取需要分析的字段
	 */
	private List<Scan> initScan(Job job) {
		...
		// 可以看出qualifer就是列祖下面的一个小属性
		// org.apache.hadoop.hbase.filter.CompareFilter.CompareOp compareOp, byte[]
		// value);
		fList.addFilter(new SingleColumnValueFilter(Bytes.toBytes(EventLogConstants.EVENT_LOGS_FAMILY_NAME),
	...
	private Filter getMultipleClumnPrefixFilter(String[] columns) {
		if (columns == null) {
			return null;
		}
